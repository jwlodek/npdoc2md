...
def validate_paths(input_path: Path, output_path: Path) -> None:
    ...
    if not input_path.exists():
        ...
        raise ValueError(f"Input path '{input_path}' is a file but does not have a .py extension.")
    ...
    if not output_path.exists():
        try:
            ...
            raise PermissionError(f"Could not create output directory at '{output_path}': {e}")
    elif output_path.is_file():
        raise NotADirectoryError(f"Output path '{output_path}' is a file, expected a directory.")
    elif not os.access(output_path, os.W_OK):
        raise PermissionError(f"Output path '{output_path}' is not writable.")


def get_target_output_file_path(input_file: Path, input_base_path: Path, output_base_path: Path) -> Path:
    ...
    relative_path = input_file.relative_to(input_base_path) if input_base_path.is_dir() else Path(input_file.name)
...
def get_cls_and_func_defined_in_module(module: ModuleType) -> tuple[dict[str, type], dict[str, Callable]]:
